...
function PriceHint({ label, value, reasons }: { label: string; value: number | null; reasons: string[] }) {
  if (value === null) {
    return (
      <div className="content-wrap rounded-[22px] border border-ink/8 bg-white/92 px-4 py-4 text-sm text-ink/78 shadow-sm">
        <p>{label}</p>
        <p className="mt-2 text-ink/62">暫不提供</p>
      </div>
    );
  }

  return (
    <details className="hint-card content-wrap rounded-[22px] border border-ink/8 bg-white/94 px-4 py-4 text-sm text-ink/78 shadow-sm">
      <summary className="hint-summary flex cursor-pointer list-none items-start justify-between gap-3">
        <div>
          <p className="text-ink/72">{label}</p>
          <p className="numeric mt-2 text-2xl font-semibold text-ink">{value.toFixed(2)}</p>
        </div>
        <span className="rounded-full border border-ink/10 bg-ink/6 px-3 py-1 text-xs font-medium text-ink/82">點開看理由</span>
      </summary>
      <div className="mt-3 space-y-2 border-t border-ink/8 pt-3 text-xs leading-6 text-ink/78">
        {reasons.map((reason) => (
          <p key={`${label}-${reason}`} className="soft-chip content-wrap rounded-2xl px-3 py-2 text-ink/82">
            {reason}
          </p>
        ))}
        <p>
...
export default function StockTradePlanPanel({ tradePlan }: StockTradePlanPanelProps) {
  ...
  return (
    <section className="glass-card rounded-[30px] p-6">
      ...
      <div className="mt-4 rounded-[22px] border border-ink/8 bg-white/94 px-4 py-4 text-sm text-ink/82 shadow-sm">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="rounded-full border border-ink/8 bg-foam px-3 py-1 text-xs text-ink/78">{activePlan.description}</span>
        </div>
        <p className="mt-3 leading-7 text-ink/86">{activePlan.preferredSide !== '觀望' ? `${activePlan.preferredSide}主劇本: ` : ''}{activePlan.summary}</p>
      </div>
      ...
      <MetricDisclosure
        className="content-wrap mt-4 rounded-[22px] bg-[linear-gradient(135deg,#08131b,#184862)] px-4 py-4 text-sm text-white shadow-lg"
        ...
        summary={<p className="text-white/96">{activePlan.riskRewardRatio !== null ? `${activePlan.primarySetup?.side ?? '主劇本'}估算風險報酬比 ${activePlan.riskRewardRatio}。` : '目前暫無可用的風險報酬比。'}</p>}
      />

      {activePlan.alternateSetup ? (
        <div className="content-wrap mt-4 rounded-[22px] border border-ink/8 bg-white/94 px-4 py-4 text-sm leading-7 text-ink/82 shadow-sm">
          <p className="font-medium text-ink">備用{activePlan.alternateSetup.side}劇本</p>
          <p className="mt-2 text-ink/84">{activePlan.alternateSetup.summary}</p>
